import numpy as np
from scheduling.utils.hopcroft_karp_matching import maximum_matching
...
def cal_cost_matrix(agents, targets, other_costs):
    ...
    for i in range(k):
        for j in range(k):
            distance_matrix[i, j] = distance(agents[i][1], agents[i][0], targets[j][1], targets[j][0]) + other_costs[i]
